...
```{r}
epu <- read_excel("/home/jovyan/data/Canada_Policy_Uncertainty_Data.xlsx",sheet=1, col_types = c("text","text","numeric"))
head(epu)
```
...
# my_data2 <- read_excel("Canada_Policy_Uncertainty_Data.xlsx",sheet=2)#, c("text","text","numeric","numeric","numeric"))
# head(my_data2)
```
...
```{r}
complete_sequence = seq(from = as.Date(min(epu$date)), to = as.Date(max(epu$date)),  by = "month")
...
ggplot(data = epu, aes(x = epu)) + geom_histogram() + xlab('epu values')
```
...
```{r}
fig<-ggplot(epu, aes(x = Index, y = epu)) + geom_line()
ggplotly(fig)
#fig
```
...
```{r}
fig<-ggplot(epu, aes(x = Index, y = epu)) + geom_line() + xlim(2000,2018)
#ggplotly(fig)
fig
```
