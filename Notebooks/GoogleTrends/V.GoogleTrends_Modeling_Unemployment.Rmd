...
unemployment_rate <- filter(unemployment_rate, 
                         date<as.Date('2020-06-01'))
```
...
```{r}
model0 %>% residuals() %>% Box.test(lag=12, fitdf=1, type="Lj")
```
...
Model 1.0
```{r}
model1 <- Arima(unemployment_rate$uer_log, order=c(1,0,0), seasonal=c(1,0,0),
