...
```{r}
head(filter(dashboard_tables[[4]], Trade == "Export",
...
```{r}
head(filter(dashboard_tables[[4]], Trade == "Import",
            Seasonal_adjustment == "Seasonally adjusted",
            Basis == "Balance of payments"))
```
...
```{r}
head(filter(dashboard_tables[[8]], Seasonal_adjustment == "Seasonally adjusted",
...
```{r}
head(filter(dashboard_tables[[9]], Type_of_operation ==
...
same table as 9
```{r}
head(filter(dashboard_tables[[10]], Type_of_operation ==
              "Total itinerant movements",
            Domestic_and_international_itinerant_movements ==
...
same table as 9
```{r}
head(filter(dashboard_tables[[11]],
...
```{r}
vectors <- c("v65201210", "v41690973", "v2062809", "v1001827265",
